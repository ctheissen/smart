...
spectrum = np.load(data_path + sci_data_name+'.npy', allow_pickle=True).item()
...
def lnlike(theta):
	...
	## Parameters MCMC
	#print('THETA:', theta)
	teff, metal, co, kzz, rv, A, lsf = theta #N noise prefactor
	...
	model = model_fit.makeModel(teff=teff, logg=5, metal=metal, co=co, kzz=kzz, rv=rv, flux_mult=A, #wave_offset=B, 
...
def lnprior(theta, limits=limits):
	...
	teff, metal, co, kzz, rv, A, lsf = theta
...
teff_mcmc, metal_mcmc, co_mcmc, kzz_mcmc, rv_mcmc, A_mcmc, lsf_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), 
	zip(*np.percentile(triangle_samples, [16, 50, 84], axis=0)))


print(teff_mcmc, metal_mcmc, co_mcmc, kzz_mcmc, rv_mcmc, A_mcmc, lsf_mcmc)
...
print('Best-fit params:', teff, z, co, kzz, rv, A, lsf)
...
#	fontsize=15)
plt.figtext(0.89,0.82,"$Teff \, {0}^{{+{1}}}_{{-{2}}}/ logg \, {3}/ [M/H] \, {4}^{{+{5}}}_{{-{6}}}/ co \, {7}^{{+{8}}}_{{-{9}}}/ kzz \, {10}^{{+{11}}}_{{-{12}}}/RV \, {13}^{{+{14}}}_{{-{15}}}$".format(\
	round(teff_mcmc[0],1),
	round(teff_mcmc[1],3),
	round(teff_mcmc[2],3),
	round(logg),
