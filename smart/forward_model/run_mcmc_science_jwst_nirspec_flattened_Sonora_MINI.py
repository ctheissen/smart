...
#sys.exit()
spectrum = np.load('data/HD13724.npy', allow_pickle=True).item()
...
wave  = spectrum['wave'] * 10000 # convert microns to angstroms
flux  = spectrum['spec']  ### NEED TO CONVERT THESE UNITS!
noise = spectrum['err']   ### NEED TO CONVERT THESE UNITS!
Cinv  = spectrum['Cinv']  ### NEED TO CONVERT THESE UNITS!
valid = spectrum['valid']  ### NEED TO CONVERT THESE UNITS!
scale = (1*u.W/u.m**2/u.micron).to(u.erg/u.s/u.cm**2/u.angstrom)

#ind   = np.where( (flux > 0) & (~np.isnan(flux) ) )
mask   = np.where( (np.isnan(flux) ) )[0]
wave  *= u.angstrom
flux  *= u.W/u.m**2/u.micron #* 1e6 * u.Jy.cgs 
noise *= u.W/u.m**2/u.micron #* 1e6 * u.Jy.cgs 
Cinv  /= scale.value**2
...
plt.show()
'''
data        = smart.Spectrum(flux=flux.value, wave=wave.value, noise=noise.value, path=data_path, order=order, instrument=instrument)
data.Cinv   = Cinv
data.valid  = valid
...
# 						'lsf_min':1,                                'lsf_max':200 				
# 					}
# elif 'sonora-2025-mini' in modelset.lower():
# elif  modelset.lower() == 'SONORA_2025_MINI':
	...
	limits         = { 
						...
						'B_min':-0.6,								'B_max':0.6,
						'N_min':0.10,                               'N_max':2.0, 	
				        'lsf_min':5,                                'lsf_max':200, # see JWST/NIRSpec resolving power plot			
					}
...
def lnlike(theta):
	...
	# teff, logg, rv, A, N, lsf = theta ## this is for running btsettlo8
	# teff, logg, metal, fsed, rv, A, lsf = theta #N noise prefactor. ### this is for sonora2024 
	...
	### this is for btsettl08
	model = model_fit.makeModel(teff=teff, logg=logg, metal=metal, co=co, kzz=kzz, rv=rv, flux_mult=A, #noise=N, #wave_offset=B, 
		lsf=lsf, order=str(data.order), data=data, modelset=modelset, include_fringe_model=include_fringe_model, 
		instrument=instrument, tell=False, smoothbreads=True, mask=mask, continuum=False)
	
    # #### for Sonora 2025 MINI grid
    # model = model_fit.makeModel(teff=teff, logg=logg, metal=metal, co=co, kzz=kzz, rv=rv, 
    #     flux_mult=A, noise=N, lsf=lsf, order=str(data.order), data=data, modelset=modelset, 
    #     include_fringe_model=include_fringe_model, instrument=instrument, tell=False, smoothbreads=True, mask=mask, continuum=False)
	...
	#print('CHI', chisquare)
	data_err_total = data.noise#*N   # ( data_err_val**2 + (err_jit * data_flux_val)**2 )**(0.5)
	
	#chisquare = np.nansum( ( model.flux - data.flux )**2/(data_err_total)**2 )

	#print(data.flux.shape)
	#print(model.flux.shape)
	#print(data.valid.shape)
	v          = (data.flux - model.flux)[data.valid]
	nonan      = np.logical_not(np.isnan(v))
	v          = v[nonan]
	Cinv_nonan = data.Cinv[nonan,:][:,nonan]
	covchi2    = np.linalg.multi_dot([v.T, Cinv_nonan, v])
	#print('ChiTotCov2', covchi2)
...
def lnprior(theta, limits=limits):
	...
	metal, co, kzz, rv, A, lsf = theta ## added in N before lsf for BTSETTL08 
...
######edits for btsetll08
# add in N 
# to run sonora you add fsed after metal and rv after theta definition
# and limits['metal_min'] < metal < limits['metal_max'] \ ## comment this out for btsettl08 goes in between log and rv 
# and limits['fsed_min']  < fsed  < limits['fsed_max'] \ ## comment back in for sonora 2024 goes between metal and rvmin
...
pos = [np.array([	# priors['teff_min']  + (priors['teff_max']   - priors['teff_min'] ) * np.random.uniform(), 
					...
					priors['metal_min'] + (priors['metal_max']  - priors['metal_min'] ) * np.random.uniform(), ## comment out when using btsettl08
					priors['co_min'] + (priors['co_max']  - priors['co_min'] ) * np.random.uniform(), ## comment out when using btsettl08
					priors['kzz_min'] + (priors['kzz_max']  - priors['kzz_min'] ) * np.random.uniform(), ## comment out when using btsettl08
					# priors['fsed_min']  + (priors['fsed_max']   - priors['fsed_min'] ) * np.random.uniform(), ## comment out when using btsettl08
					...
					#priors['N_min']     + (priors['N_max']      - priors['N_min'])     * np.random.uniform(), ## comment back in for sonora 2024
...
ylabels = [#"$T_{\mathrm{eff}} (K)$",
           #"$\log{g}$(dex)",
           "[M/H]",## comment in for sonorq2024
		   "CO",
		   "Kzz",
        #    "fsed",## comment in for sonorq2024
           "$RV(km/s)$",
           "$C_{F_{\lambda}}$ (cnt/s)",
           #"$C_\mathrm{Noise}$",
           "LSF"
...
file_log2.write("kzz_mcmc {}\n".format(str(kzz_mcmc[0])))##comment in for sonora2024
# file_log2.write("fsed_mcmc {}\n".format(str(fsed_mcmc[0])))##comment in for sonora2024
...
#file_log2.write("N_mcmc {}\n".format(str(N_mcmc[0])))#comment bsck out for  sonora2024
...
#### for btsettl08
model = model_fit.makeModel(teff=teff, logg=logg, metal=z, co=co, kzz=kzz, rv=rv, flux_mult=A, #noise=N,
...
print("SAVING TABLE")
Twrite.write(save_to_path + '/JWST_NIRSpec_model_%s.csv'%modelset, overwrite=True)
print("TABLE SAVED")
...
###### for sonora2024
plt.figtext(0.89,0.82,"$Teff \, {0}^{{+{1}}}_{{-{2}}}/ logg \, {3}^{{+{4}}}_{{-{5}}}/ [M/H] \, {6}^{{+{7}}}_{{-{8}}}/ CO \, {9}^{{+{10}}}_{{-{11}}}/ Kzz \, {12}^{{+{13}}}_{{-{14}}}/ RV \,{15}^{{+{16}}}_{{-{17}}}$".format(\
	teff,
	0,
	0,
	round(5,1),
