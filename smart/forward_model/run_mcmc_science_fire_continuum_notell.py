...
def lnlike(theta, data, lsf0):
	...
	teff, logg, rv, N, lsf = theta #N noise prefactor
	...
	#print(mask)
	#print('1')
	model = smart.makeModel(teff=teff, logg=logg, rv=rv, #flux_mult=A, #wave_offset=B, 
...
teff_mcmc, logg_mcmc, rv_mcmc, N_mcmc, lsf_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), 
	zip(*np.percentile(triangle_samples, [16, 50, 84], axis=0)))


print(teff_mcmc, logg_mcmc, rv_mcmc, N_mcmc, lsf_mcmc)
...
model = model_fit.makeModel(teff=teff, logg=logg, rv=rv, #flux_mult=A,
	lsf=lsf, order=str(data.order), data=data, modelset=modelset, 
...
#	fontsize=15)
plt.figtext(0.89,0.82,"$Teff \, {0}^{{+{1}}}_{{-{2}}}/ logg \, {3}^{{+{4}}}_{{-{5}}}/ RV \, {6}^{{+{7}}}_{{-{8}}}$".format(\
	round(teff_mcmc[0]),
	round(teff_mcmc[1]),
	round(teff_mcmc[2]),
	round(logg_mcmc[0],1),
