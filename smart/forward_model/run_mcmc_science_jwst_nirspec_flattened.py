...
#sys.exit()
spectrum = np.load('data/HD 13724 redo.npy', allow_pickle=True).item()
...
# JWST-specific: use N_max=10.0 (or 2.0 for some modelsets)
limits = model_limits.get_model_limits(modelset, priors, logg_max, A_const=A_const, N_max=10.0)

# JWST-specific limit overrides
if limits is not None:
	# Most JWST limits need wider RV range and lsf parameters
	if 'btsettl08' in modelset.lower() or 'sonora' in modelset.lower() or modelset.lower() == 'phoenixaces':
		limits['rv_min'] = -1000.0
		limits['rv_max'] = 1000.0
		# Add lsf limits if not already present
		if 'lsf_min' not in limits:
			limits['lsf_min'] = 1
			limits['lsf_max'] = 200
	
	# JWST-specific overrides for btsettl08
	if 'btsettl08' in modelset.lower():
		limits['teff_max'] = min(priors['teff_max'] + 300, 3500)
		limits['vsini_max'] = 10.0
		limits['A_min'] = -50
		limits['A_max'] = 50
		limits['B_min'] = -50
		limits['B_max'] = 50
	
	# JWST-specific overrides for phoenix-btsettl-cifist2011-2015
	elif modelset.lower() == 'phoenix-btsettl-cifist2011-2015':
		limits['teff_min'] = max(priors['teff_min'] - 300, 1200)
		limits['teff_max'] = min(priors['teff_max'] + 300, 3500)
		limits['vsini_max'] = 10.0
		limits['A_min'] = -50
		limits['A_max'] = 50
	...
	# JWST-specific overrides for sonora (not sonora-2024)
	elif 'sonora' in modelset.lower() and modelset.lower() != 'sonora-2024':
		limits['vsini_max'] = 10.0
		limits['A_min'] = -50
		limits['A_max'] = 50
		limits['B_min'] = -50
	...
	# sonora-2024 already has correct N_max=2.0 from model_limits
