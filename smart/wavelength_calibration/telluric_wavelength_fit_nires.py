...
def wavelengthSolutionFit(data, model, order, **kwargs):
	...
	# getting the parameters of initial wavelength solution 
		...
		if i == 0: # compute initial values for the coefficients
			...
			#coeffs = np.polyfit(pixel, data.wave[pixel_range_start:pixel_range_end], deg=4)
	...
	if save is True:
		...
		if instrument.lower() == 'nires': 
			...
			print('WAVE shape', wave_array.shape)
			#WAVE [18917.07444734 18919.93040896 18922.78634604 ... 24662.44827843 24665.24930865 24668.05031238]
			#FINAL WAVELENGTH SOLUTION: [18917.07444734 18919.93040896 18922.78634604 ... 24589.61218364 24592.4139038  24595.21559738]
...
def run_wave_cal(data_name, data_path, order_list,
	...
	for order in order_list:
		...
		#cal_param_dict = {'ALL':{'xcorr_range':5.0, 'outlier_rej':2.5, 	'pixel_range_start':0, 	'pixel_range_end':-1}}
		...
		airmass  = float(round(data.header['AVE_AM']*2)/2)
		...
		if instrument in ['nirspec','nires']:
		...
		if instrument.lower() == 'nires': 
			...
			print(np.nanmax(telluric.flux), np.nanmedian(telluric.flux))
			data     = smart.continuumTelluric(data=telluric_new, model=telluric, instrument=instrument)
			print(np.nanmax(data.flux), np.nanmedian(data.flux))
			## constant offset correction
			const    = np.nanmedian(telluric_new.flux) - np.nanmedian(telluric.flux)
			telluric_new.flux -= const
			print(np.nanmax(telluric_new.flux), np.nanmedian(data.flux))
			print('TEST'*10)
		'''
