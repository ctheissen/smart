...
import sys
import smart
from .cal_param import cal_param_fire
...
def pixelWaveShift(dataN, model, start_pixel, window_width=40, delta_wave_range=20,
	...
	length1           = kwargs.get('length1', len(dataN.oriWave))
	pixel             = kwargs.get('pixel', np.delete(np.arange(length1), dataN.mask))
	...
	for i in np.arange(-delta_wave_range, delta_wave_range+step, step):
		...
		if len(dataN.bestshift) > 1:
			j = dataN.bestshift[counter] + i
		else:
			j = i	
		xcorr = smart.xcorrTelluric(dataN, model, j, start_pixel, window_width, lsf)
	...
	plt.plot(model2.wave, model2.flux, label='model')
	plt.show()
	'''
	...
	if test:
		...
		ax1.plot(dataN.wave, dataN.flux, color='black', linestyle='-', 
		...
		ax1.set_ylim(0,1.1)
		ax1.set_xlim(dataN.wave[0]-10, dataN.wave[-1]+10)
		...
		ax1.axvline(x=dataN.wave[start_pixel], linestyle='--', color='blue',
			        linewidth=linewidth)
		ax1.axvline(x=dataN.wave[start_pixel+window_width], linestyle='--',
			        color='blue', linewidth=linewidth)
		ax1.get_xaxis().get_major_formatter().set_scientific(False)
		ax1.legend()
		ax1t = ax1.twiny()
		ax1t.set_xlim(0, len(dataN.oriWave)-1)
		ax1t.set_xlabel("Pixel")

		ax2.plot(dataN.wave, dataN.flux, color='black', linestyle='-', 
				 label='telluric data', alpha=0.5, linewidth=linewidth)
		...
		ax2.set_ylim(0,1.1)
		ax2.set_xlim(dataN.wave[start_pixel]-0.1,
					 dataN.wave[start_pixel+window_width]+0.1)
		ax2.axvline(x=dataN.wave[start_pixel], linestyle='--',
					color='blue', linewidth=linewidth)
		ax2.axvline(x=dataN.wave[start_pixel+window_width], linestyle='--',
					color='blue', linewidth=linewidth)
		ax2.get_xaxis().get_major_formatter().set_scientific(False)
		ax2.legend()
		ax2t = ax2.twiny()
		ax2t.set_xlim(dataN.wave[start_pixel]-0.1,
					  dataN.wave[start_pixel+window_width]+0.1)
		ax2t.set_xticks([dataN.wave[start_pixel],
					  	 dataN.wave[start_pixel+window_width]])
...
def wavelengthSolutionFit(data, model, order, **kwargs):
	...
	pixel0             = np.delete(np.arange(len(data.wave)), np.union1d(data.mask, mask_custom).astype(int) )
	#pixel0             = np.arange(length1)
	#if mask_custom != []:
	#pixel              = pixel0
	pixel              = pixel0#[pixel_range_start:pixel_range_end]
	print("Pixel Ranges:", pixel_range_start, pixel_range_end)
	print('len(pixel)', len(pixel), len(pixel0))
	...
	# getting the parameters of initial wavelength solution 
		...
		#	#sys.exit()
			...
			spec_range          = len(pixel) # window range coverage for xcorr
			width_ranges        = np.arange(pixel_range_start, spec_range+pixel_range_start-width, step_size)-pixel_range_start
			width_range_centers = np.arange(pixel_range_start, spec_range+pixel_range_start-width, step_size)-pixel_range_start + width//2
			widths              = np.zeros(len(width_ranges), dtype=int) + width
		...
		if i == 0: # compute initial values for the coefficients

			print('****COMPUTING INITIAL WAVELENGTH SOLUTION****')
			
			# Fit a 5th-order polynomial: wavelength = f(pixel)
			#print(len(pixel), len(data.wave), len(data.wave[pixel_range_start:pixel_range_end]))
			coeffs = np.polyfit(pixel, data.wave, deg=4)

			# Convert to a polynomial function
			poly_func = np.poly1d(coeffs)

			#print(coeffs)
			data2.header['C0'] = coeffs[4]
			data2.header['C1'] = coeffs[3]
			data2.header['C2'] = coeffs[2]
			data2.header['C3'] = coeffs[1]
			data2.header['C4'] = coeffs[0]
			data2.header['POPT0'] = coeffs[4]
			data2.header['POPT1'] = coeffs[3]
			data2.header['POPT2'] = coeffs[2]
			data2.header['POPT3'] = coeffs[1]
			data2.header['POPT4'] = coeffs[0]
		...
		for counter, j, center, width in zip(range(len(widths)), width_ranges, width_range_centers, widths):
			print(i+1, counter, j, center, width, step)
		...
		print('NEW WAVE SOL!!!', new_wave_sol, new_wave_sol.shape)
		new_wave_sol0 = waveSolution(pixel0, c0, c1, c2, c3, c4)
		print('NEW WAVE SOL0!!!', new_wave_sol0, new_wave_sol0.shape)
		...
		ax1.set_ylim( np.nanmin(model3.flux)*0.8, np.nanmax(model3.flux)*1.2)
		ax1.get_xaxis().get_major_formatter().set_scientific(False)
		ax1.legend(frameon=False)
		print('Data Ylim:',np.min(data3.flux), np.max(data3.flux))
		print('Model Ylim:',np.min(model3.flux), np.max(model3.flux))
		#sys.exit()
	...
	if save is True:
		...
		if instrument.lower() == 'fire': 
			...
			print('WAVE', waveSolution(np.arange(length1), c0, c1, c2, c3, c4))
			print('WAVE shape', waveSolution(np.arange(length1), c0, c1, c2, c3, c4).shape)

			wave_array = waveSolution(np.arange(length1), c0, c1, c2, c3, c4)

			# Create an empty primary HDU (must be HDU 0)
			primary_hdu = fits.PrimaryHDU()

			# Create an ImageHDU to store the array as extension 1
			image_hdu  = fits.ImageHDU(data=wave_array, name='WAVELENGTH')

			# Create the full HDUList and write it
...
def run_wave_cal(data_name, data_path, order_list,
	             ...
	             xcorr_step=0.1, niter=20, outlier_rej=None, defringe_list=[62], cal_param=None, instrument='nirspec'):
	"""
	...
	for order in order_list:
		...
		#cal_param_dict = {'20':{'xcorr_range':60.0, 'outlier_rej':2.5, 	'pixel_range_start':875, 	'pixel_range_end':-1}}
		...
		if cal_param is not None:
			...
			xcorr_range       = cal_param_fire[str(order)]['xcorr_range']
			pixel_range_start = cal_param_fire[str(order)]['pixel_range_start']
			pixel_range_end   = cal_param_fire[str(order)]['pixel_range_end']
			if outlier_rej is None:
				outlier_rej = cal_param_fire[str(order)]['outlier_rej']
		...
		print(data_name)
		print(data_path)
		hdu           = fits.open(data_path + data_name + '.fits')
		print(hdu.info())
		print('ORDER:', order)
		header = hdu[1].header
		
		flux   = hdu[1].data['FX'][order].flatten()
		noise  = np.sqrt(hdu[1].data['VAR'][order].flatten()) # maybe VAR
		wave   = hdu[1].data['WAVE'][order].flatten()
		#flux   = hdu[1].data['FX'].flatten()
		#noise  = np.sqrt(hdu[1].data['VAR'].flatten()) # maybe VAR
		#wave   = hdu[1].data['WAVE'].flatten()

		data        = smart.Spectrum(flux=flux, wave=wave, noise=noise, 
			                         oriFlux=flux, oriWave=wave, oriNoise=noise, instrument=None, order=None)
		data_unflat = smart.Spectrum(flux=flux, wave=wave, noise=noise, 
			                         oriFlux=flux, oriWave=wave, oriNoise=noise, instrument=None, order=None)
		data.header = header
		### mask the end pixels
		#pixel_start, pixel_end = 60900, -1
		...
		#for i in data.header: print(i, data.header[i])

		#data        = smart.Spectrum(name=data_name, order=order, path=data_path, apply_sigma_mask=apply_sigma_mask, instrument=instrument)
		#data_unflat = smart.Spectrum(name=data_name, order=order, path=data_path, apply_sigma_mask=apply_sigma_mask, instrument=instrument)
		data.bestshift=[]
		...
		print('PIXEL_RANGE:', pixel_range_start, pixel_range_end)
		print(data.wave, len(data.wave))
		print(len(data.wave[pixel_range_start:pixel_range_end]))
		if apply_sigma_mask:
			...
			if apply_edge_mask:
				...
				data.oriFlux  = np.delete(data.oriFlux, mask_custom)[pixel_range_start: pixel_range_end]
				data.oriWave  = np.delete(data.oriWave, mask_custom)[pixel_range_start: pixel_range_end]
				data.oriNoise = np.delete(data.oriNoise, mask_custom)[pixel_range_start: pixel_range_end]
			else:
		...
		print(data.wave, len(data.wave), length1)
		data1    = copy.deepcopy(data)
		print(data1.wave, len(data1.wave), length1)
		#plt.plot(data1.wave, data1.flux)
		#plt.show()
		#sys.exit()
		...
		data     = smart.continuumTelluric(data=data, model=model, instrument=instrument)
		print('CHECK THESE ARE THE SAME')
		print(len(data1.wave), len(data.wave))
		...
		# plotting
		pixel       = np.delete(np.arange(len(data.wave)), np.union1d(data.mask, mask_custom).astype(int) )
		pixel       = pixel#[pixel_range_start:pixel_range_end]
		...
		#if apply_sigma_mask:
		lsf   = smart.getLSF(telluric_new, continuum=False)#, test=True, save_path=save_to_path)
		#else:
		...
		if instrument.lower() == 'fire': 
			print('TEST'*10)
			print(np.nanmax(data.flux), np.nanmedian(data.flux))
			print(np.nanmax(telluric_new.flux), np.nanmedian(telluric_new.flux))
			data     = smart.continuumTelluric(data=data, model=telluric_new, instrument=instrument)
			print(np.nanmax(data.flux), np.nanmedian(data.flux))
			## constant offset correction
			const    = np.nanmedian(data.flux) - np.nanmedian(telluric_new.flux)
			data.flux -= const
			print(np.nanmax(data.flux), np.nanmedian(data.flux))
			print('TEST'*10)
		'''
		...
		print('OLD WAVELENGTH SOLUTION:', data1.wave)
		print('OLD WAVELENGTH SOLUTION:', data1.wave.shape)
		print('FINAL WAVELENGTH SOLUTION:', new_wave_sol)
		print('FINAL WAVELENGTH SOLUTION:', new_wave_sol.shape)